...
int parse_input_advanced(char *input, char **args) {
    ...
    while (i < strlen(input) && arg_count < MAX_ARGS - 1) {
        ...
        if ((input[i] == '&' || input[i] == '|') && i + 1 < strlen(input) && input[i + 1] == input[i] && !in_quotes) {
            if (j > 0) {
                current_arg[j] = '\0';
                args[arg_count++] = strdup(current_arg);
                j = 0;
...
void execute_command_with_redirection(char **args, int background) {
    ...
    if (pid == 0) {
        ...
        // Expand glob patterns in arguments - create new array with expansions
        char *exec_args[MAX_ARGS];
        int exec_idx = 0;
        int has_glob = 0;
        
        for (int i = 0; args[i] != NULL && exec_idx < MAX_ARGS - 1; i++) {
            // Check if argument contains glob characters
            if (strchr(args[i], '*') || strchr(args[i], '?') || (strchr(args[i], '[') && strchr(args[i], ']'))) {
                has_glob = 1;
                glob_t glob_result;
                int ret = glob(args[i], GLOB_NOCHECK | GLOB_TILDE, NULL, &glob_result);
                
                if (ret == 0 && glob_result.gl_pathc > 0) {
                    for (size_t j = 0; j < glob_result.gl_pathc && exec_idx < MAX_ARGS - 1; j++) {
                        exec_args[exec_idx++] = glob_result.gl_pathv[j];
                    }
                    // Note: we don't free glob_result here as we're about to exec
                } else {
                    exec_args[exec_idx++] = args[i];
                }
            } else {
                exec_args[exec_idx++] = args[i];
            }
        }
        exec_args[exec_idx] = NULL;
...
// Execute command with && and || operators (conditional execution)
void execute_conditional_chain(char **args, int background) {
    int i = 0;
    int success = 1;  // Track if previous command succeeded
    
    // First, check if there are any && or || operators
    ...
    if (!has_conditional) {
        // No conditional operators, just execute normally
        execute_command(args, background);
        return;
    }
    
    // Parse and execute conditional chains
    while (i < MAX_ARGS && args[i] != NULL) {
        // Find the next operator or end of arguments
        int cmd_end = i;
        char *next_operator = NULL;
        
        while (cmd_end < MAX_ARGS && args[cmd_end] != NULL) {
            if (strcmp(args[cmd_end], "&&") == 0 || strcmp(args[cmd_end], "||") == 0) {
                next_operator = args[cmd_end];
                break;
            }
            cmd_end++;
        }
        
        // Create array for this command
        char *cmd_args[MAX_ARGS];
        int cmd_len = 0;
        for (int j = i; j < cmd_end && args[j] != NULL; j++) {
            cmd_args[cmd_len++] = args[j];
        }
        cmd_args[cmd_len] = NULL;
        
        // Execute command based on previous result
        if (success || (next_operator && strcmp(next_operator, "||") == 0)) {
            // Execute if: (a) success and next is &&, (b) !success and next is ||, (c) no operator yet
            if (cmd_len > 0) {
                execute_command(cmd_args, background);
                success = (last_exit_status == 0);
            }
        }
...
void shell_loop() {
    ...
    while (1) {
        ...
        if (arg_count > 0) {
            ...
            execute_conditional_chain(args, background);
            free_args(args);
        }
    }
}
